  ...
  async install(targetIds: string[], onProgress?: (progress: IccceInstallProgress) => void): Promise<IccceInstallResult[]> {
    ...
    for (const group of groups.values()) {
      ...
      if (closeFailed) {
        ...
        for (const candidate of group) results.push({ targetId: candidate.id, ok: false, action: "failed", message: "ICC-CE 未能正常退出，未安装插件；请关闭后重试" });
        continue;
      }
      ...
      try {
        ...
        for (const candidate of group) results.push({ targetId: candidate.id, ok: !launchFailed, action: launchFailed ? "failed" : "installed", message: launchFailed ? `插件已写入，但 ICC-CE 自动${restarting ? "重启" : "启动"}失败，请手动启动` : restarting ? `已安装 ICC-CE 插件 v${packageData.version}，ICC-CE 已自动重启` : `已安装 ICC-CE 插件 v${packageData.version}，ICC-CE 已自动启动`, version: packageData.version });
      } catch (error) {
        for (const candidate of closed) await restart(candidate.executablePath, candidate.launchArgs).catch(() => undefined);
