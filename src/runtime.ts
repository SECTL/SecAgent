...
/**
 * The model receives the schemas of every explicitly enabled MCP tool and decides which to call.
...
export class SecAgentRuntime {
  ...
  async run(input: string): Promise<RunResult> {
    ...
    this.emit("mcp.tools/list", mcpTools.map((tool) => ({ key: tool.key, server: tool.server, name: tool.name, description: tool.description, inputSchema: tool.inputSchema })));
    ...
    const message = await this.agent.run(input, tools, async (key, args) => this.callTool(input, key, args));
  ...
  private async callTool(request: string, key: string, args: Record<string, unknown>): Promise<unknown> {
