...
export function SettingsApp() {
  ...
  const [activePage, setActivePage] = useState(() => {
    ...
    const builtInPage = ["settings-wake", "settings-tts", "settings-models", "settings-mcp", "settings-plugins", "settings-system", "settings-updates", "settings-telemetry"].includes(hash);
  ...
  return <main className={`settings-shell has-window-title ${isOobe ? "oobe-shell" : ""} ${activePage === "settings-plugins" ? "plugin-settings-shell" : ""} ${bridge.platform === "darwin" ? "macos-settings" : ""} ${bridge.platform !== "darwin" ? "windows-settings" : ""}`}>
    ...
    <section id="settings-tts" className={`settings-section ${isOobe || activePage === "settings-tts" ? "settings-section-active" : ""}`}><div className="section-title"><div><h2>朗读</h2></div></div>
      <article className="settings-card"><div className="form-grid"><label>语音音色<SelectCombobox ariaLabel="语音音色" value={settings.tts.voice} options={ttsVoices.map(([value, label]) => ({ value, label }))} onChange={(voice) => setSettings((current) => current && { ...current, tts: { ...current.tts, voice } })} /></label><label>语速<SelectCombobox ariaLabel="语速" value={settings.tts.rate} options={ttsRates.map(([value, label]) => ({ value, label }))} onChange={(rate) => setSettings((current) => current && { ...current, tts: { ...current.tts, rate } })} /></label></div></article>
    </section>
