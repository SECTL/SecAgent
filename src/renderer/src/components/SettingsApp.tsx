...
export function SettingsApp() {
  ...
  return <main className={`settings-shell has-window-title ${isOobe ? "oobe-shell" : ""} ${bridge.platform === "darwin" ? "macos-settings" : ""} ${bridge.platform === "win32" ? "windows-settings" : ""}`}>
    ...
    {!isOobe && <nav className="settings-nav" aria-label="语音识别设置导航"><button type="button" className={activePage === "settings-speech" ? "active" : ""} aria-current={activePage === "settings-speech" ? "page" : undefined} onClick={() => { setActivePage("settings-speech"); window.history.replaceState(null, "", "#settings-speech"); }}>语音识别</button></nav>}
    {isOobe && <>
