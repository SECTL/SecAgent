...
export function SettingsApp() {
  ...
  return <main className={`settings-shell has-window-title ${isOobe ? "oobe-shell" : ""} ${activePage === "settings-plugins" ? "plugin-settings-shell" : ""} ${bridge.platform === "darwin" ? "macos-settings" : ""} ${bridge.platform === "win32" ? "windows-settings" : ""}`}>
    ...
    {bridge.platform === "win32" && <section id="settings-system" className={"settings-section " + (activePage === "settings-system" ? "settings-section-active" : "")}><div className="section-title"><div><h2>系统</h2><p>管理 SecAgent 是否随 Windows 登录自动启动。</p></div></div>
      <article className="settings-card"><label className="toggle-row"><span className="toggle-copy"><strong>开机自启</strong><small>开启后，Windows 登录时会在后台启动 SecAgent；需要使用时可从托盘打开主窗口。</small></span><input type="checkbox" checked={settings.autostart === true} onChange={(event) => setSettings((current) => current && { ...current, autostart: event.target.checked })} /></label><label className="toggle-row nested-toggle-row"><span className="toggle-copy"><strong>开机自启后隐藏主窗口</strong><small>开机自启动时只在托盘后台运行，语音唤醒和全局快捷键照常可用；关闭后开机自启会直接打开主窗口。</small></span><input type="checkbox" checked={settings.autostartHidden !== false} onChange={(event) => setSettings((current) => current && { ...current, autostartHidden: event.target.checked })} /></label></article>
    </section>}
